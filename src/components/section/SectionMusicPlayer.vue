...
function setProgress(event: Event) {
  ...
  musicStore.setProgress(Number.parseFloat(target.value))
}

// 设置音量
function setVolume(event: Event) {
  const target = event.target as HTMLInputElement
  musicStore.setVolume(Number.parseFloat(target.value))
}
</script>
...
<template>
  <Section title="音乐播放器" relative>
    ...
    <div flex items-center justify-between mb-4 gap-4>
      <!-- 当前歌曲信息 -->
      <div flex-1 min-w-0>
        <h3 text-lg font-bold mb-1 text-gray-800 truncate>
          {{ musicStore.currentTrack.name }}
        </h3>
        <p text-gray-600 text-xs m-0>
          {{ musicStore.currentTrackIndex + 1 }} / {{ musicStore.musicList.length }}
        </p>
      </div>
    ...
    <div v-show="musicStore.showPlaylist" border-t border-gray-200 pt-4>
      <h4 m-0 mb-3 text-gray-800 text-sm font-medium>
        播放列表
      </h4>
