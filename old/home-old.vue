<script>
import { DATA_DICT } from '@/assets/js/datadict'
import { ID_NAME_DICT } from '@/assets/js/id_operator'
import { Hero } from '@/assets/js/page'
import Worker from '@/assets/js/saveScore.worker.js?worker'
import TableComponent from '@/assets/js/table'
import TransferComponent from '@/assets/js/transfer'
import geostats from 'geostats'
import palette from 'google-palette'

import '@/styles/base.css'

import '@/styles/old/compare.css'
import '@/styles/old/transfer.css'
import '@/styles/old/table.css'
...
export default {
  ...
  mounted() {
    ...
    for (const key in this.DICT_PIC_URL) {
      let hero = new Hero(key, this.cache_hero_dict[key])
  ...
  methods: {
    ...
    nclassesListener() {
      let nclasses = Number.parseInt(this.nclasses_input.value, 10)
    ...
    flush_self(sort_by = 'win_rate', desc = true) {
      ...
      for (let i = 0; i < this.star6_staff_amount; i++) {
        ...
        selfStr += `<tr style="color: #${color_list[i]}; background-color: currentColor"><td class="final_table_text">${this.cup_size[Number.parseInt(i / this.star6_staff_amount_div)]}       </td><td class="final_table_text">${this_rank}</td><td class="final_table_text">${chr_name}</td><td class="final_table_text">${win_rate}%</td><td       class="final_table_text" style="text-align: right; padding-right: 25px">${scores}</td></tr>`
      }
    ...
    view_self() {
      let self_table = document.getElementById('self_table')
      let self_button = document.getElementById('self_rst_button')
      if (this.self_close) {
    ...
    close_or_view() {
      ...
      if (this.close_or_view_flag) {
        ...
        document.getElementById('您已投票').innerText = ''
        let table = document.getElementById('final_order_table')
    ...
    async view_final_order() {
      ...
      xhr.onreadystatechange = function () {
        if (xhr.readyState === 4 && xhr.status === 200) {
          ...
          self.clusterList = rate_list.map(r => Number.parseFloat(r))
    ...
    get_color_list(cluster_list) {
      ...
      palette('rainbow', cluster_list.length, 0, 0.5, 0.95).forEach((color, i) => {
        color_list.push.apply(color_list, Array.from({ length: cluster_list[i].length }).fill(color))
      })
      return color_list
    },
    ...
    get_cluster_list(data_array, bound_list) {
      let i; let j; let k = 0; let first; let last
    ...
    formatTime(timestamp) {
      ...
      mon = Number.parseInt(mon) < 10 ? `0${mon}` : mon
      day = Number.parseInt(day) < 10 ? `0${day}` : day
      hour = Number.parseInt(hour) < 10 ? `0${hour}` : hour
      min = Number.parseInt(min) < 10 ? `0${min}` : min
      sec = Number.parseInt(sec) < 10 ? `0${sec}` : sec
...
&nbsp;
                  ...
                  <div class="function">
                    <input
                      id="self_rst_button" class="result" type="button" value="查看您的投票结果"
                      @click="view_self()"
                    >
                  </div>
                  <div class="function">
                    <input
                      id="vote_1v1_matrix_trigger" class="result" type="button" value="查看1v1对位矩阵"
              ...
              <td>
                <div class="operator-container" @click="save_score_right()">
                  <img id="right_png">
                  <div id="right_png_name" class="zh" />
                </div>
              </td>
            </tr>
          </table>
        ...
        <div class="footer">
          <table style="width: 100%">
            <tr>
              <td>
                <h3 class="move-up-animation red_text">
              ...
              <td>
                ...
                <audio id="audio" ref="audio" loop :src="playSrc" preload="none" />
              </td>
            </tr>
          </table>
        </div>
        ...
        <div class="footer">
          ...
          <h2 class="move-up-animation" style="animation-delay: 0s;">
            规则：每次投票，得票+1分，未得票-1分，最后统计所有干员的胜率和得分，以胜率排名。
          </h2>
          <h2 class="move-up-animation" style="animation-delay: 0.2s;">
            每个ip只能投100票，自第101票起，每票按0.01票计算权重。
          </h2>
          <h2 class="move-up-animation" style="animation-delay: 0.4s;">
      ...
      <div class="block">
        <div class="footer">
          ...
          <h2 class="move-up-animation red_text" style="animation-delay: 0.6s;">
            警告：由于干员随机抽取，次数并不完全一样，且数据全部来源于网友投票，结果可能存在不少谬误。
          </h2>
          <h2 class="move-up-animation red_text" style="animation-delay: 0.8s;">
            仅供娱乐，请勿用作抽卡建议和TTK工具！
          </h2>
          <h2 class="move-up-animation" style="animation-delay: 0.8s;">
            出于服务器资源管理考虑，实时投票结果并不会实时存储，刷票等行为导致服务器崩溃将丢失投票结果。
          </h2>
          <h2 class="move-up-animation" style="animation-delay: 1s;">
            鸣谢：本投票两两比较的灵感来源于NGA，制作过程中得到了PRTS.wiki，血狼破军和各位群友、网友的支持和建议，在此向各位表示感谢！
          </h2>
          <h4 class="move-up-animation" style="animation-delay: 1.2s;">
            本投票反映干员经过实战检验，口碑稳定后的强度评价，请投票前充分考虑合约、肉鸽n12以上、演算陌域n4以上，日常S关。
          </h4>
          <h4 class="move-up-animation" style="animation-delay: 1.4s;">
            “自定义聚类数目”功能：聚类数目越多，区分度越高(图中以颜色区分) 默认区分度为 80%
          </h4>
          <h4 class="move-up-animation" style="animation-delay: 1.6s;">
            若要尝试不同的聚类数目，可以在右下方的“分层数”中调整。
          </h4>
          <br>

          <div class="link-container">
            <h3 class="move-up-animation" style="animation-delay: 1.8s;">
              本项目已开源至GitHub仓库：
          ...
          <div class="link-container">
            <h3 class="move-up-animation" style="animation-delay: 2s;">
              第三次6星强度投票结果：(截至艾拉)
          ...
          <div class="link-container">
            <h3 class="move-up-animation" style="animation-delay: 2.2s;">
              6星厨力投票结果：
          ...
          <div class="link-container">
            <h3 class="move-up-animation" style="animation-delay: 2.4s;">
              第二次6星强度投票结果：(截至锏)
          ...
          <div class="link-container">
            <h3 class="move-up-animation" style="animation-delay: 2.6s;">
              第一次6星强度投票结果：(截至涤火杰西卡)
          ...
          <div class="link-container">
            <h3 class="move-up-animation" style="animation-delay: 2.8s;">
              萨米收藏品投票结果：(不含DLC)
...
&nbsp;
          ...
          <table id="self_table" class="final_table">
            ...
            <thead>
              <tr>
                ...
                <th>干员代号</th>
                <th>
                  胜率<span id="win_rate" class="sort-icon select down" @click="sort_table" />
                </th>
                <th>
                  得分<span id="scores" class="sort-icon down" @click="sort_table" />
                </th>
              </tr>
            </thead>
            <tbody id="self_order_tbody" />
          </table>
        </div>
      </div>
      ...
      <div id="vote_1v1_matrix_container" class="footer">
        <div id="operators-1v1-transfer" />
        <input class="operators-1v1-button" type="button" value="查看所选对位数据" @click="calculate_operators_1v1_matrix()">
        <input class="operators-1v1-button" type="button" value="清空全部数据" @click="clear_operators_1v1_matrix()">
        <div id="operators-1v1-table" />
      </div>
      ...
      <button id="topBtn" @click="scrollToTop()">
        回到顶部
      </button>
      <button id="refreshBtn" onclick="view_final_order()">
        刷新总榜数据
      </button>
